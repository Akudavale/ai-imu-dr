...
from train_torch_filter import avg_loss_per_epoch
...
def results_filter(args, dataset):
    ...
    avg_RMSE_xy = np.mean( np.concatenate(RMSE_xy).ravel())
    avg_RMSE_z = np.mean(np.concatenate(RMSE_z).ravel())
    print(f"avg RMSE loss xy is {avg_RMSE_xy} and on z is {avg_RMSE_z}")

    # Define folder paths
    base_folder = 'layer_3_AI-IMU_Dead-Reckoning\\ai-imu-dr-master\\results\\arbier_results_pkl'
    RMSExy_folder = os.path.join(base_folder, 'RMSE_XY.pkl')
    RMSEz_folder = os.path.join(base_folder, 'RMSE_Z.pkl')

    # Save average RMSE values to pickle files
    with open(RMSExy_folder, 'wb') as f:
        pickle.dump(avg_RMSE_xy, f)

    with open(RMSEz_folder, 'wb') as f:
        pickle.dump(avg_RMSE_z, f)
    
    #2)import saved loss per epoch and plot the graph (epoch,loss)
        
    file_path = 'layer_3_AI-IMU_Dead-Reckoning\\ai-imu-dr-master\\results\\arbier_results_pkl\\avg_loss_results.pkl'
