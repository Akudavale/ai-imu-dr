...
def train_filter(args, dataset):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        save_iekf(args, iekf)
        print(f"loss train for one epoch is {loss_train} and scalar value is {loss_train.item()}")
        print("Amount of time spent for 1 epoch: {}s\n".format(int(time.time() - start_time)))
        start_time = time.time()

    save_loss_folder='layer_3_AI-IMU_Dead-Reckoning\\ai-imu-dr-master\\results\\arbier_results_pkl\\avg_loss_results.pkl' 
